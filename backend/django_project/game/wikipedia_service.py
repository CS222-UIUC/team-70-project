...
class WikipediaService:
    ...
    def get_random_articles(cls, count=1):
        ...
        try:
            ...
            if "query" in data and "random" in data["query"]:
                ...
                logger.error(
                    f"Unexpected response format from Wikipedia API: {data}"
                )
                return []
    ...
    def get_article_content(cls, pageid):
        ...
        try:
            ...
            if "parse" not in data:
                logger.error(
                    f"Error getting article content, unexpected response: {data}"
                )
                return None
            ...
            if "text" in data["parse"] and "*" in data["parse"]["text"]:
                ...
                article_data["content"] = cls._extract_text_from_html(
                    html_content
                )

            # Get image URLs if available
            if "images" in data["parse"]:
                article_data["images"] = cls._get_image_urls(
                    data["parse"]["images"]
                )
    ...
    def _get_image_urls(cls, image_filenames):
        ...
        params = {
            ...
            # Limit to 10
            "titles": "|".join(
                ["File:" + img for img in filtered_images[:10]]
            ),
            "prop": "imageinfo",
            "iiprop": "url",
        }
