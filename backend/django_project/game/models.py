...
class UserProfile(models.Model):
    ...
    def calculate_average_score(self):
        ...
        if self.total_games_played > 0:
            ...
            if scores.exists():
                self.average_score = scores.aggregate(models.Avg("score"))[
                    "score__avg"
                ]
                self.save()
...
class DailyScore(models.Model):
    ...
    # The title of the article they were guessing
    article_title = models.CharField(max_length=255, blank=True)
    last_guess = models.CharField(
    ...
    def calculate_score(self):
        ...
        base_score = 1000
        # Penalty for time: max 500 points
        time_penalty = min(500, self.time_taken / 10)
        # Penalty per guess: 50 points each, max 300
        guess_penalty = min(300, self.guesses * 50)
        # Penalty per hint: 40 points each, max 200
        hint_penalty = min(200, self.hints_used * 40)

        # Calculate final score
        self.score = max(
            0, base_score - time_penalty - guess_penalty - hint_penalty
        )
