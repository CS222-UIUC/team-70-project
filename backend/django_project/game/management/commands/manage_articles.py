...
class Command(BaseCommand):
    help = 'Manage Wikipedle articles (select daily, cleanup, etc.)'
    ...
    def handle(self, *args, **options):
        ...
        if not any([options['select_daily'], 
    ...
    def _select_articles_for_days(self, days_ahead):
        ...
        for i in range(days_ahead):
            ...
            daily_article, created = ArticleService.ensure_daily_article(target_date)
    ...
    def _cleanup_old_articles(self, max_age_days=30):
        ...
        deleted_count = ArticleService.cleanup_old_articles(max_age_days, preserve_used=True)
    ...
    def _clear_all_articles(self):
        ...
        confirm = input("Are you sure you want to delete ALL cached articles? This action cannot be undone. (y/N): ")
        if confirm.lower() != 'y':
            self.stdout.write(self.style.WARNING("Aborted."))
            return

        remove_daily = input("Also remove daily article entries that block deletion? (y/N): ").lower() == 'y'
        
        if remove_daily:
            DailyArticle.objects.all().delete()
            self.stdout.write(self.style.WARNING("All DailyArticle entries removed."))
        ...
        self.stdout.write(self.style.SUCCESS(f"Deleted {count} articles from cache."))
